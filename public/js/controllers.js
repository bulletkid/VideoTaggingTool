...
videoTaggingAppControllers
    ...
    .factory('state', ['$http', '$rootScope', function ($http, $rootScope) {
        ...
        $rootScope.jobSetup = {
            ...
            regionSizes: { values: Array.apply(null, Array(100)).map(function (item, i) {
                return i + 1
            }), default: 20 }
        };
        ...
        $scope.submit = function () {
            ...
            var data = {
                ...
                configJson: {
                    ...
                    tags: ($scope.tags && $scope.tags.split(',').map(function (tag) {
                        return tag.trim();
                    })) || ''
                }
            };
...
//                        videoCtrl.src = jobData.video.Url;
        ...
        $scope.change_frame = function(step){
            ...
            $scope.currentFrame = $scope.currentFrame + step;
            if ($scope.currentFrame < 1){
                $scope.currentFrame = 1;
                return;
            }
            console.log("Frame length is - ", Object.keys($scope.frames).length, $scope.frames);
            if ($scope.currentFrame > $scope.frames.length){
                $scope.currentFrame = $scope.frames.length;
                return;
            }
            $scope.img_src = $scope.videoImageSrcBaseUrl + $scope.currentFrame.toString() + ".jpg";
        }
